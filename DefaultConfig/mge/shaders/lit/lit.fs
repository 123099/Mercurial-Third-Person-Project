...
vec4 calculateLight(int index, vec4 normal)
{
	...
	return vec4((ambientAndDiffuse.xyz + specular.xyz) * vec3(lights[index].intensity * calculateAttenuation(index) * spotFalloff * shadowAttenuation), (ambient.a + diffuse.a + specular.a) * 0.34);
}
